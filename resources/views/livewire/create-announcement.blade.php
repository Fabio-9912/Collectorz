<div style="background: #4B6C8B" id="editForm" class="edit">
    <x-message />
    <div class="register-dark addAnn container">
        <form wire:submit="store" enctype="multipart/form-data" class="announcement pb-5">
            {{-- <x-message /> --}}
            <div class="illustration d-flex flex-row justify-content-center align-items-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                    <g fill="none" stroke="#4B6C8B" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="1.5">
                        <path
                ...
                {{-- <h2 class="text-white text-center ms-2 d-none d-md-block insertAnn">
                    @if (request()->routeIs('announcements.create'))
                        {{ __('ui.addAnnouncement') }}
                    @endif
                </h2> --}}
            </div>
            <div class="col-12 d-flex flex-column flex-md-row mb-4 px-0">
                <div class="form-group col-12 col-md-5 ps-md-0 me-md-2">
                    <label>{{ __('ui.announcementTitle') }}</label>
                    <input class="form-control" wire:model.blur='title' type="text"
                        placeholder="{{ __('ui.announcementTitle') }}">
                    @error('title')
                        <span class="message">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group col-12 col-md-7 pe-md-0">
                    <select wire:model="category_id" class="col-12 h-100">
                        <option value="Scegli una categoria">{{ __('ui.selectCategory') }}</option>
            ...
            <div class="col-12 d-flex flex-column flex-md-row px-0">
                <div class="col-12 col-md-5 form-group ps-md-0">
                    <div class="form-group col-12 me-2 px-0">
                        <label class="textareaLabel">{{ __('ui.announcementDescription') }}</label>
                        <textarea wire:model.blur='description' cols="30" rows="10"
                            placeholder="{{ __('ui.announcementDescription') }}"></textarea>
                        @error('description')
                            <span class="message">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group col-12 me-2 px-0">
                        <label><svg xmlns="http://www.w3.org/2000/svg" class="bg-trasparent" width="18"
                                height="18" viewBox="0 0 64 64">
                                <ellipse cx="10.97" cy="52.578" fill="currentColor" rx="1.742"
                                    ry="1.188" />
                        ...
                        <input class="form-control" wire:model.blur='price' type="number" min="0.00" max="99999999"
                            placeholder="{{ __('ui.price') }}">
                        @error('price')
                            <span class="message">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group d-none d-md-block col-12 px-0">
                        <button class="btn btn-primary btn-block mt-0 w-100 submitRequest" type="submit"
                            onclick="document.querySelector('.sendRequest').classList.add('spinner-border', 'spinner-border-sm'); window.scroll(0,0)">
                            <span class="sendRequest me-2" role="status" aria-hidden="true"></span>
                            {{ __('ui.submitAnnouncement') }}
                        </button>
                    </div>
                </div>
                <div class="col-md-7 pe-md-0 ms-md-2">
                    <div class="row" style="height:fit-content;">
                        <div class="mb-3">
                            <button type="button" class="btn btn-primary btn-block mt-0 w-100"
                                onclick="document.querySelector('.form-controll').click()">{{ __('ui.selectImg') }}</button>
                            <input wire:model="temporary_images" type="file" name="images" multiple
                                class="form-controll d-none w-100 shadow @error('temporary_images') is-invalid @enderror"
                                placeholder="img">
                            @error('temporary_images')
                                <span class="message text-danger mt-2">{{ $message }}</span>
                            @enderror
                            @error('images')
                                <span class="message text-danger mt-2">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="row">
                        @if (!@empty($images))
                            <div class="d-flex flex-row flex-wrap rounded py-1 boxImages">
                                @foreach ($images as $key => $image)
                                    <div class="col-8 col-md-4 d-flex align-items-center mb-2">
                                        <div class="img-preview mx-auto shadow rounded me-1"
                                            style="background-image: url({{ $image->temporaryUrl() }});">
                                        </div>
                                        <button type="button" class="btnDelImg text-center"
                                            wire:click="removeImage({{ $key }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em"
                                                class="" viewBox="0 0 1024 1024">
                                                <path fill="#ff0000"
            ...
            <div class="form-group col-12 d-md-none my-2">
                <button class="btn btn-primary btn-block mt-0 w-100 submitRequest" type="submit"
                    onclick="document.querySelector('.sendRequest').classList.add('spinner-border', 'spinner-border-sm'); window.scroll(0,0)">
                    <span class="sendRequest" role="status"
                        aria-hidden="true">{{ __('ui.submitAnnouncement') }}</span>
                </button>
            </div>
        </form>
    </div>
